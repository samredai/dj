...
async def test_get_all_cubes(
    ...
    # Get only available cubes and test that this cube is excluded
    response = await client_with_repairs_cube.get("/cubes?include_only_available=true")
    assert response.is_success
    data = response.json()
    ...
    # Get only available cubes and test that this cube is now included
    response = await client_with_repairs_cube.get("/cubes?include_only_available=true")
    assert response.is_success
    data = response.json()
    assert len([cube for cube in data if cube["name"] == "default.repairs_cube_9"]) == 1
